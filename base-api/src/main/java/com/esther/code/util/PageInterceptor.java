...
public class PageInterceptor implements Interceptor {
	...
	private void setParameters(PreparedStatement ps, MappedStatement mappedStatement, BoundSql boundSql, Object parameterObject)
		 ...
		 * mappedStatement.getParameterMap().getId()); List<ParameterMapping>
		 * parameterMappings = boundSql.getParameterMappings(); if
		 * (parameterMappings != null) { Configuration configuration =
		 * mappedStatement.getConfiguration(); TypeHandlerRegistry
		 * typeHandlerRegistry = configuration.getTypeHandlerRegistry();
		 * MetaObject metaObject = parameterObject == null ? null:
		 * configuration.newMetaObject(parameterObject); for (int i = 0; i <
		 ...
		 * parameterMapping.getProperty(); PropertyTokenizer prop = new
		 * PropertyTokenizer(propertyName); if (parameterObject == null) { value
		 * = null; } else if
		 * (typeHandlerRegistry.hasTypeHandler(parameterObject.getClass())) {
		 * value = parameterObject; } else if
		 * (boundSql.hasAdditionalParameter(propertyName)) { value =
		 * boundSql.getAdditionalParameter(propertyName); } else if
		 * (propertyName.startsWith(ForEachSqlNode.ITEM_PREFIX)&&
		 * boundSql.hasAdditionalParameter(prop.getName())) { value =
		 * boundSql.getAdditionalParameter(prop.getName()); if (value != null) {
		 * value =
		 * configuration.newMetaObject(value).getValue(propertyName.substring
		 * (prop.getName().length())); } } else { value = metaObject == null ?
		 * null : metaObject.getValue(propertyName); } TypeHandler typeHandler =
		 * parameterMapping.getTypeHandler(); if (typeHandler == null) { throw
		 * new
		 * ExecutorException("There was no TypeHandler found for parameter "+
		 * propertyName + " of statement "+ mappedStatement.getId()); } try{
		 * typeHandler.setParameter(ps, i + 1, value,
		 * parameterMapping.getJdbcType()); }catch(SQLException e){
		 * e.printStackTrace(); } } } }
		 */
	}
	...
	private String generatePageSql(String sql, Paging page) {
		if (page != null && dialect != null) {
			...
			if ("mysql".equals(dialect)) {
				pageSql.append(sql);
				pageSql.append(" limit " + currentPage * page.getPageSize() + "," + page.getPageSize());
